...
export const ControlHubNode: React.FC<NodeProps<RobotNode>> = ({ id, data }) => {
  ...
  return (
    <div className="bg-slate-900 border-2 border-amber-500 rounded-lg shadow-2xl w-[34rem] text-slate-200 font-mono text-[11px] select-none">
      ...
      <div className="bg-amber-950/50 border-b border-amber-500/30 px-4 py-2 flex items-center justify-between rounded-t-md">
        ...
        <div className="flex items-center gap-1.5 bg-slate-950 px-2 py-0.5 rounded border border-slate-800 shrink-0">
          ...
          <span className="text-[10px] text-slate-400 font-sans">IMU: <span className="text-emerald-400 font-bold">OK</span></span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-6 p-4 bg-slate-950/40">
        ...
        {/* LEFT BANK: INPUTS (TARGET HANDLES) */}
        ...
        {/* RIGHT BANK: POWER & ACTUATORS (SOURCE HANDLES) */}
        <div className="space-y-4 pl-1">
          <div>
            <span className="text-[10px] text-amber-500 uppercase tracking-wider font-sans block mb-1.5 font-bold">DC Motors</span>
            <div className="space-y-1">
              {[0, 1, 2, 3].map((motor) => (
                <div key={`motor_${motor}`} className="flex items-center justify-between bg-slate-900/60 px-2 py-1 rounded relative border border-slate-800/40">
                  <span className="text-slate-300 font-bold">Port {motor}</span>
